...
// Pegando os valores passados por parametro:
    ...
    if (alteraAnuncio($conexao, $id, $nome, $placa, $modelo, $cor, $quilometragem, $preco, $ano, $combustivel, $opcionais, $observacoes
    ,$nome_imagem1, $nome_imagem2, $nome_imagem3, $idmarca,$idcategoria, $idcidade, $data)) {
       header("Location: verAnuncio.php?id={$id}");
        die();
    } else {
        $msg = mysqli_error($conexao);
        ?>
        <p class="text-danger">O anúncio <?= $nome ?> não foi alterado: <?= $msg ?></p>
        <?php
    }
}
    include ("../Rodape.php");
